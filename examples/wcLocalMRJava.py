...
def wordCountJob():
    ...
    mapDesc = { "executable": "java",
                  "arguments": ['-cp $PWD/*', 'WcMapper'],
                  "number_of_processes": 1,
                  "spmd_variation":"single",
                  "files" : ['ssh://localhost/' + os.getenv("HOME") + "/WcMapper.class", 
                             'ssh://localhost/' + os.getenv("HOME") + "/pmr.jar"]
                }
    
    reduceDesc = { "executable": "java",
                   "arguments": ['-cp $PWD/*', 'WcReducer'],
                   "number_of_processes": 1,
                   "spmd_variation":"single",
                   "files" : ['ssh://localhost/' + os.getenv("HOME") + "/WcReducer.class",
                              'ssh://localhost/' + os.getenv("HOME") + "/pmr.jar"]
                 }
